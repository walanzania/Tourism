...
<?php 
	...
	// Updating
		...
		if ($updStory) {
			...
			echo "<script>success_noti('Operation Succeeded ...'); window.location='create_tour_story?categ_unit=$slug'</script>";
		}else {
			echo "<script>error_noti('Error Occured !!!')</script>";
		}
	}
	...
	if (isset($_POST['changeImage'])) {
		...
		if (move_uploaded_file($tmp_dir, $upload_dir.$newFile)) {
			...
			if ($updImg) {
				echo "<script>success_noti('Operation Succeeded ...'); window.location='create_tour_story?categ_unit=$cu'</script>";
			}else{
				echo "<script>error_noti('Error Occured !!!')</script>";
			}
		}
