<?php 
	...
	if (isset($_GET['publish'])) {
		...
		if ($updStatus) {
			echo "<script>alert('Updated Successfully ...');window.location='story_details?categ_unit=$slug'</script>";
		}else{
	...
	if (isset($_GET['delete'])) {
		...
		$deleteStory = $connect2db->prepare("DELETE FROM tbl_tour_story WHERE id = ?");
		$deleteStory->execute([$story_id]);
		if ($deleteStory) {
			echo "<script>alert('Story Deleted');window.location='all_story';</script>";
		}else{
			echo "<script>error_noti('Error Deleting !!!')</script>";
		}
	}
